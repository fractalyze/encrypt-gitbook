...
The [**Tensor Product**](https://en.wikipedia.org/wiki/Tensor_product) is an operation that generates all possible combinations of two vectors v and u from different vector spaces. For example, if $$\bm{q_1} = [a, b]$$ and $$\bm{q_2} = [x, y, z]$$, the tensor product between the two is the following matrix:

$$
\bm{q_1} \otimes \bm{q_2} =
\begin{bmatrix}
...
$$
\text{Given an evaluation point } \bm{r} \in \mathbb{F}^l\text{ and }z \in \mathbb{F}^{2^l} \\
\begin{align*}
g(\bm{r}) &= \langle \otimes_{i=1}^{l} (r_1, 1 - r_1), z\rangle \\
...
$$
\begin{align*} 
  g(\bm{r}) &= \mathsf{eq}((0, 0), \bm{r}) \cdot z_1 + \mathsf{eq}((0, 1), \bm{r}) \cdot z_2 + \mathsf{eq}((1, 0), \bm{r}) \cdot z_3 + \mathsf{eq}((1, 1), \bm{r}) \cdot z_4 \\
...
1. Each matrix $$\bm{u}$$ is provided as an input, where $$\bm{u_i}$$ represents the $$i$$-th row.
...
$$
\bm{\hat{u}}= \{\mathsf{Enc}(\bm{u_i})\}_{i \in [r]} \in (\mathbb{F}^N)^r
$$
...
$$
\bm{u'} = \mathsf{RLC}(\bm{u}) =  \sum_{i=1}^r\alpha^{i-1} \cdot \bm{u_i} \in \mathbb{F}^c
$$
...
$$
\sum_{i=1}^r\alpha^{i-1} \cdot \bm{\hat{u}_i} \stackrel{?}= \mathsf{Enc}(\bm{u'})
$$
...
1. The verifier samples $$\bm{q_1} \in \mathbb{F}^r$$.
2. The prover computes $$\bm{u''}$$ and sends it to the verifier: (Identical to Testing Phase 2.)

$$
u'' = \sum_{i=1}^rq_{1,i}\cdot u_i \in \mathbb{F}^c
$$

3. The verifier tests the following equality by sampling $$\bm{q_2} \in \mathbb{F}^c$$: (Identical to Testing Phase 3.) Here, tensor product between $$\bm{q_1}$$ and $$\bm{q_2}$$ are flattened to the vector.
...
$$
g(\bm{r}) = \langle(\bm{q_1} \otimes \bm{q_2}), \bm{u})\rangle \stackrel{?}= \langle \bm{u''}, \bm{q_2}\rangle
$$
...
$$
\begin{align*}
  \langle(\bm{q_1} \otimes \bm{q_2}), u\rangle &= q_{1,1}\cdot q_{2,1} \cdot u_{1,1} + q_{1,1}\cdot q_{2,2}\cdot u_{2,1} + q_{1,2}\cdot q_{2,1}\cdot u_{1,2}+  q_{1,2}\cdot q_{2,2}\cdot u_{2,2} \\
  &= (q_{1,1}\cdot u_{1,1} + q_{1,2}\cdot u_{1,2})\cdot q_{2,1} + (q_{1,1}\cdot u_{2,1} + q_{1,2} \cdot u_{2,2})\cdot q_{2,2} \\
  &= \langle\sum_{i=1}^{2} \bm{q_{1,i}} \cdot u_i, \bm{q_2}\rangle
  \end{align*}
$$
...
The figure above illustrates the linear-time encoding method used in Brakedown. For a given rate $$\rho$$, $$\text{Enc}(\bm{x})$$ is a function that generates a vector of length $$n \cdot \rho^{-1}$$ from an input vector $$\bm{x}$$ of length $$n$$. The output is divided into three parts: $$(\bm{x}, \bm{z}, \bm{v})$$. The encoding $$\text{Enc}(\bm{x})$$ proceeds as follows:
...
For the encoded result $$\bm{w} = \mathsf{Enc}(\bm{x})$$, given the parameter $$\beta$$, the zero norm $$\|\bm{w}\|_0$$ and distance $$\delta$$ are given by:

$$
\|\bm{w}\|_0 > \beta n\text{, } \delta =  (\beta n) / (n\rho^{-1}) = \beta / r
$$

The zero norm, also known as[ Hamming weight](https://en.wikipedia.org/wiki/Hamming_weight), represents the number of non-zero elements in $$\bm{x}$$:

$$
\|\bm{x}\|_0 = \#\{i \, | \, x_i \neq 0\}
$$

The execution time of encoding is proportional to the input vector $$\bm{x}$$'s length. Proofs related to this encoding method are detailed in Section 5.1 of the paper.
...
The sparse representation is defined using $$\bm{R}$$, $$\bm{C}$$ and $$\bm{V}$$ as the following:\
1\. Each row of $$\bm{R}$$,$$\bm{C}$$,$$\bm{V}$$ defines a single non-zero entry in the sparse matrix.
...
$$
D(\bm{r_x}, \bm{r_y}) = \sum_{\bm{k} = \{0,1\}^{\log N}} \mathsf{val}(\bm{k})\cdot \widetilde{\mathsf{eq}}(b^{-1}(\mathsf{row}(\bm{k})), \bm{r_x})\cdot \widetilde{\mathsf{eq}}(b^{-1}(\text{col}(\bm{k})), \bm{r_y}), \\
\text{where }\mathsf{val}(\bm{k}) = V_k, \mathsf{row}(\bm{k}) = R_k, \mathsf{col}(\bm{k}) = C_k, \\ b^{-1}: \text{canonical injection from }\mathbb{F} \text{ to } \{0, 1\}^{\log M}, \\
\widetilde{\mathsf{eq}}(\bm{x}, \bm{y}) = 
\begin{cases}
    1,& \text{if } \bm{x} = \bm{y}\\
    0,& \text{otherwise}
\end{cases}
...
$$
\forall \bm{k} \in \{0, 1\}^{\log N},  \space E_{r_x}(\bm{k}) = \widetilde{\mathsf{eq}}(b^{-1}(\mathsf{row}(\bm{k})), r_x) \\
\forall \bm{k} \in \{0, 1\}^{\log N},  \space E_{r_y}(\bm{k}) = \widetilde{\mathsf{eq}}(b^{-1}(\mathsf{col}(\bm{k})), r_y) \\
$$

2. The prover and verifier perform a sumcheck protocol for the equation:

$$
v \stackrel{?}= \sum_{\bm{k} \in \{0, 1\}^{\log N}} \mathsf{val}(\bm{k}) \cdot E_{r_x}(\bm{k}) \cdot E_{r_y}(\bm{k})
$$

3. At the final round of sumcheck, the verifier checks the following:

$$
v(\bm{r_z}) \stackrel{?}= v_{\text{val}} \land E_{r_x}(\bm{r_z}) \stackrel{?}= v_{E_{r_x}} \land E_{r_y}(\bm{r_z}) \stackrel{?}= v_{E_{r_y}}
$$

To verify the prover's claims of $$E_{r_x}$$ and $$E_{r_y}$$, an **Offline Memory Checking** method is employed (refer to "_Detour: offline memory checking_" in Section 6 of the paper for details).
...
$$
\bm{A}\cdot \bm{Z} \circ \bm{B} \cdot \bm{Z} = \bm{C} \cdot \bm{Z}\text{, where } \bm{A}, \bm{B}, \bm{C} \in \mathbb{F}_q^{M\times M}\text{ and } \bm{Z} \in \mathbb{F}_q^M
$$

This can be converted into a sumcheck format as follows (simplified for illustration; see Section 7 for details):

$$
0 \stackrel{?}= \sum_{\bm{x} \in \{0, 1\}^{\log M}} \widetilde{\text{eq}}(\bm{r}, \bm{x}) \cdot [(\sum_{\bm{y} \in \{0, 1\}^{\log M}} \widetilde{A}(\bm{x}, \bm{y})\cdot \widetilde{Z}(\bm{y}))\cdot (\sum_{\bm{y} \in \{0, 1\}^{\log M}} \widetilde{B}(\bm{x}, \bm{y})\cdot \widetilde{Z}(\bm{y})) - (\sum_{\bm{y} \in \{0, 1\}^{\log M}} \widetilde{C}(\bm{x}, \bm{y})\cdot \widetilde{Z}(\bm{y}))]
$$
